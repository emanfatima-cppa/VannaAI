    ...
    def get_sql_prompt(self, question: str, question_sql_list, ddl_list, doc_list, **kwargs):
        ...
        if schema_block:
            constraint = (
                ...
                "the schema. When the user mentions a name or value, use LIKE with wildcards "
                "on the appropriate column.\n"
                "3. Never use SELECT *. Always name columns explicitly.\n\n"
                "4. For text searches, always use the SQL LIKE operator with wildcards "
                "instead of exact string matching. For example, use "
                "`c.MtCommitteeHeader_Name LIKE '%13AP Com%'` rather than "
                "`c.MtCommitteeHeader_Name = '13AP Com'`, unless the user explicitly "
